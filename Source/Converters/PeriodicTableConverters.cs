...
{
	public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture) => Application.Current.FindResource($"Colour{(((bool?)values[0]) is not null ? (bool)values[0] ? (int)(Category?)values[6] : ((int)(Block?)values[9] * 2 - 1) : values[1] is not null ? ((int?)values[1]) < ((double?)values[7]) ? values[7] is not null ? 1 : 0 : values[8] is not null ? ((int?)values[1]) < ((double?)values[8]) ? 3 : 5 : 0 : values[2] is not null ? ((Category?)values[2]) == ((Category?)values[6]) ? (int)(Category?)values[6] : 0 : values[3] is not null ? ((Block?)values[3]) == ((Block?)values[9]) ? (int)(Block?)values[9] * 2 - 1 : 0 : values[4] is not null ? ((Group?)values[4]) == ((Group?)values[10]) ? 1 : 0 : (((Period?)values[5]) == ((Period?)values[11]) ? 1 : 0))}");
